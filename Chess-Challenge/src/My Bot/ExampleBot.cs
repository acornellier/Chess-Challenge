...
public class ExampleBot : IChessBot
{
    ...
    public int Evaluate(Board board)
    {
        ...
        foreach (var stm in new[] { true, false })
        {
            for (var p = PieceType.Pawn; p <= PieceType.King; p++)
            {
                int piece = (int)p, ind;
                var mask = board.GetPieceBitboard(p, stm);
                while (mask != 0)
                {
    ...
    public int Search(Board board, Timer timer, int alpha, int beta, int depth, int ply)
    {
        ...
        var bound = best >= beta ? 2 : best > origAlpha ? 3 : 1;
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        {
            var score = Search(board, timer, -30000, 30000, depth, 0);
