...
public class WindwallBot : IChessBot
{
    // 0 is INVALID, 1 is LOWERBOUND, 2 is UPPERBOUND, 4 is EXACT for bound

    readonly (ulong, Move, int, int, int)[] transpositionTable = new (ulong, Move, int, int, int)[0x400000];

    int[,,] historyTable;
    readonly Move[] killers = new Move[1024];
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        {
            var eval = searchBoard(chosenDepth, 0, alpha, beta);
        ...
        {
            var currentKey = board.ZobristKey;
...
#if STATS
        {
            ...
            int Search(int newAlpha, int R = 1, bool nullMovePruning = true)
            {
                return -searchBoard(depth - R, plyFromRoot + 1, -newAlpha, -alpha, nullMovePruning);
            }
            ...
            var allMoves = board.GetLegalMoves(qsearch && !inCheck);
            var moveScores = new int[allMoves.Length];
            ...
            foreach (var move in allMoves)
            {
                ...
                var importantMoves = move.IsCapture || move.IsPromotion;
                //int R = (moveCount >= 6 && depth > 2) ? (int)(2.5 + Log(depth) * Log(moveCount) / 2.7) : 1;
                var R = moveCount >= 6 && depth > 2 ? 3 : 1;
        ...
        if (ttEntry.Item3 == 0) { 
            ...
            var boundType = bestScore >= beta ? 2 : bestScore > alphaOrig ? 3 : 1;
        ...
        int Evaluate()
        {
            ...
            for (var pieceBB = board.GetPieceBitboard((PieceType)(pieceType + 1), color == 0); pieceBB != 0;)
            {
                var square = BitboardHelper.ClearAndGetIndexOfLSB(ref pieceBB) ^ (56 * color);
    ...
    readonly int[] pieceValues = { 100, 310, 330, 500, 1000, 10000, 94, 281, 297, 512, 936, 10000 };

    readonly ulong[] pieceTables =
    {
        9259542123273814144, 16357001140413309557, 8829195605423724908, 8254401669090808169, 7313418688563415655, 7384914337435197812, 6737222767702746730, 9259542123273814144,
        11081220660097301, 3987876604305901423, 5889764664614570412, 8615829970516021910, 8323936949179225464, 7599697423020169584, 7154940515368202861, 1687519861085072745,
        7170907476791297912, 7390528431378371153, 8117082644194436478, 8972740228098131838, 8830870139635010180, 9263780805260055178, 9552012216381055361, 6880781611516057963,
        11577242485982338987, 11214168400861567660, 8904630919850999184, 7527071450275215468, 6658137190229968489, 6009895851999132511, 6084482357074295353, 7886789834650901350,
        7241961428581395373, 7519176848743963830, 8318016057608023993, 7306369598957387393, 8603695488949846909, 8251286653394652805, 6735286635485691265, 9182408126746812750,
        4582289962092626573, 11348908854965197411, 8617781306342151786, 8028920214486217308, 5728408685346316109, 8246770769504399717, 9333560970455320968, 8188824274210166926,
        9259542123273814144, 18446744073709551615, 16061197207704294100, 11572154847021273489, 10198820791839654783, 9549736231487373176, 10198551484841362041, 9259542123273814144,
        5069491205526864157, 7455822975978661964, 7524541400582942039, 8035431733674084462, 7960834280560624750, 7601372000551791722, 6227482657520642388, 7155491318799420992,
        8244812703126220648, 8681963116054150258, 9401405507207856260, 9045915848980202370, 8828055359350013303, 8394015409149540721, 8245661556352184677, 7599658875216755567,
        10199125451369908357, 10055849173233928323, 9765923324499426173, 9548631222335405954, 9477131093459761269, 8971306245150767216, 8825507717624329597, 8611590021041063020,
        8617240532094257812, 8040227885603724928, 7820089199224394633, 9481933937386764708, 7970407823045732247, 8099037312892111493, 7667768075636792416, 6873735846648900695,
        3917408671579997295, 8399651535887701899, 9984928491787627661, 8689300325139585667, 7961402723962161525, 7889615596832196471, 7310895313622170479, 5430896352595961941,
    };
