...
public sealed class Board
{
    ...
    public Board(Chess.Board boardSource)
    {
        ...
        for (var i = 0; i < boardSource.AllGameMoves.Count; i++)
        {
            var move = boardSource.AllGameMoves[i];
            var movePieceType = PieceHelper.PieceType(board.Square[move.StartSquareIndex]);
            var capturePieceType = move.IsEnPassant ? PieceHelper.Pawn : PieceHelper.PieceType(board.Square[move.TargetSquareIndex]);
        ...
        for (var i = 0; i < board.pieceLists.Length; i++)
        {
    ...
    public void MakeMove(Move move)
    {
        ...
        {
            OnPositionChanged();
            board.MakeMove(new Chess.Move(move.RawValue), true);
    ...
    public void UndoMove(Move move)
    {
        if (!move.IsNull)
        {
            ...
            board.UndoMove(new Chess.Move(move.RawValue), true);
            OnPositionChanged();
            depth--;
        }
    }
    ...
    public Move[] GetLegalMoves(bool capturesOnly = false)
    {
        ...
        if (!hasCachedMoves)
        {
            var moveSpan = movesDest.AsSpan();
            moveGen.GenerateMoves(ref moveSpan, board, true);
    ...
    {
        var includeQuietMoves = !capturesOnly;
    ...
    Move[] GetLegalCaptureMoves()
    {
        ...
        {
            var moveSpan = movesDest.AsSpan();
            moveGen.GenerateMoves(ref moveSpan, board, false);
    ...
    {
        return moveGen.IsInitialized ? moveGen.InCheck() : board.IsInCheck();
    }
    ...
    {
        return IsInCheck() && HasZeroLegalMoves();
    }
    ...
    {
        return board.currentGameState.fiftyMoveCounter >= 100;
    }
    ...
    {
        return depth > 0 && repetitionTable.Contains(board.ZobristKey);
    }
    ...
    {
        return board.currentGameState.HasKingsideCastleRight(white);
    }
    ...
    {
        return board.currentGameState.HasQueensideCastleRight(white);
    }
    ...
    {
        var colIndex = white ? Chess.Board.WhiteIndex : Chess.Board.BlackIndex;
    ...
    {
        var p = board.Square[square.Index];
        var white = PieceHelper.IsWhite(p);
    ...
    /// </summary>
    public Move[] GameMoveHistory { get; }
    ...
    public string CreateDiagram(bool blackAtTop = true, bool includeFen = true, bool includeZobristKey = true, Square? highlightedSquare = null)
    {
        ...
        for (var y = 0; y < 8; y++)
        {
            ...
            for (var x = 0; x < 8; x++)
            {
                ...
                var pieceInSquare = GetPiece(square);
        ...
        static char GetPieceSymbol(Piece piece)
        {
            if (piece.IsNull)
                return ' ';
            var pieceSymbol = piece.IsKnight ? 'N' : piece.PieceType.ToString()[0];
